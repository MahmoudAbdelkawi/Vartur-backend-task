...
const uploadMiddlewareImage = upload.array("images")  /* "image"  is the name of attribute which sent from the client body Parser*/


const imageResizing = expressAsyncHandler(async(req:any,res:Response,next:NextFunction)=>{ 
	...
	await sharp(req.files.images?.data).resize(3200 , 3200).toFormat("jpeg")
